...
- [x] **🟢 /workspace + /workspaces 斜杠命令（工作区切换与列表）**：`/workspace <路径>` 校验目录存在后设置工作区偏好（持久化到 `~/.dsh/im-gateway/workspaces.json`），后续 `/new` 在该工作区创建会话；`/workspace`（不带参数）列出所有工作区及会话数（聚合 `sessionQuery.listSessions()` 按 `header.cwd` 分组）；`/workspaces` 同效（别名）。`ensureSessionInner` 创建会话时 `effectiveCwd = workspaceOf(convKey) ?? config.cwd`，`meta: { cwd: effectiveCwd }` 传给 `agents.create`。**🟡 /workspace 不带参数行为坑**：初版 `/workspace`（无参）只显示当前工作区偏好，用户期望列出所有工作区——改为调用 `handleWorkspaces` 列出全部。
- [x] **🔴 session-cleaner 游离会话误判（ID 编码不匹配）**：dsh 的 `encodeSegment`（`dsh-session-persistence-jsonl/lib/index.js:84`）把会话 ID 中的非 `[A-Za-z0-9._-]` 字符编码为 `~XXXX`（如 `:` → `~003A`），用作文件系统目录名。`session-cleaner` 的 `listSessions()` 扫描 `~/.dsh/sessions/<encoded-workspace>/<encoded-sessionId>/` 目录树，用 `entry.name`（编码后，如 `im~003Aqq-s5ey~003Ac2c~003A...`）作为会话 ID，但 `workspace.json` 的 `sessionIds` 存的是**原始 ID**（如 `im:qq-s5ey:c2c:...`）。`activeIds.has(entry.name)` 永远 false → **所有 IM 会话都被判定为游离**。修复：在 `session-store.ts` 中实现 `encodeSegment`/`decodeSegment`（与 dsh 对齐），`listSessions()` 用 `decodeSegment(entry.name)` 解码得到原始 ID 用于 `active`/`archived` 判断和显示；`deleteSession()`/`previewSession()` 用 `encodeSegment(id)` 编码后定位文件系统目录；`removeFromProjcache()` 同时匹配原始和编码两种形式。**教训：跨包读写同一存储时，必须对齐编码规则——dsh 内部的 `encodeSegment` 是文件系统安全编码，不是可选的。**
- [x] **🔴 /new 后旧会话从桌面端左侧工作区消失（dispose 过度清理）**：`/new`（`handleReset`）调用 `handle.dispose()` 销毁旧 agent。`AgentHandle.dispose()` 的文档明确写着"removes its session from the store"——它从 `ctx.sessions` 中**删除**会话。桌面端 UI 的会话列表数据源是 `sessionQuery.listSessions()`，它查两个地方：① `ctx.sessions.list()`（live sessions）② `sessionPersistence.list()`（磁盘持久化）。`dispose()` 删了 ①，而 ② 是**异步**的（`session/disposed` → `retire` → `flush` 写磁盘，`dispose` 返回时 flush 可能还没完成）。UI 恰好在这个空档刷新 → 内存没有、磁盘还没写 → 旧会话"消失"。**诊断方法**：在 `dispose` 前后记录 `workspace.json` raw/projected sessionIds + `ctx.sessions.list()` live sessions + `sessionQuery.listSessions()` 返回值，对比发现 workspace.json 不变但 live sessions 少了旧会话。**修复**：不调用 `handle.dispose()`，改调 `handle.agent.cancel({ kind: 'user' }, { keepInbox: true })` 只取消当前 turn。旧 agent 留在 `ctx.sessions`/`ctx.agents` 中，`listSessions()` 仍返回它，UI 仍显示。im-gateway 内部映射（`convToSession`/`agentHandles` 等）仍清除，不会往旧会话发消息。旧 agent 无 `followup` 调用不消耗资源。**教训：`dispose()` 是"从系统中彻底删除会话"，不是"停止使用会话"——IM 网关的 `/new` 只需要停止使用旧会话、开启新会话，不应该删除旧会话的存在记录。**
- [x] **🟡 attachToWorkspace 中 mutate 过滤移除旧会话（workspace.json sessionIds 被清理）**：`dsh-workspace` 的 `WorkspaceEntity.mutate()`（`lib/index.js:149`）在每次写入时过滤 `sessionIds`：`changed.sessionIds.filter((id) => this.host.sessionPath(id) === changed.path)`。`sessionPath(id)` 来自 `sessionPaths` Map，该 Map 在启动时通过 `indexHeaders(sessionPersistence.list())` 构建——如果某些会话的 header 不在持久化列表中（文件损坏/被删/未 flush），`sessionPaths` 不含它们，`sessionPath(id)` 返回 `undefined` ≠ `record.path`，会话被**永久从 `workspace.json` 的 `sessionIds` 中移除**。`attachSession` 调用 `mutate`，所以每次 `attachToWorkspace` 都可能清理掉旧会话。**诊断**：在 `attachSession` 前后记录 `workspace.json` raw sessionIds，对比发现旧会话被移除。**修复**：`attachToWorkspace` 中 `attachSession` 前备份 `sessionIds`，之后把被 `mutate` 移除的会话直接写回 `workspace.json`（绕过 `mutate` 的过滤）。**这是 dsh-workspace 的设计行为**（清理无效引用），但对 IM 网关来说旧会话可能只是还没 flush 到磁盘，不应该被清理。
