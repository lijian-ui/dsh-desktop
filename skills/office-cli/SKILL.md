...
## 设计风格路由（Design Style Routing）

When the user asks for a PPT with a specific visual style for academic / consulting / official (政企) / gala (红金) decks, load the matching visible-design rules from `references/designs/officecli-design-*.md`. Load ONE style; never mix.

- 学术风 / academic / research / thesis / paper / 论文 / 答辩 / 学术 -> `references/designs/officecli-design-academic.md`
- 商务 / consulting / 咨询 / strategy / 战略 / 汇报 (business) / 商业 -> `references/designs/officecli-design-consulting.md`
- 政务 / 党建 / 红金 / gala / official / 政企 / 大会 / 仪式 -> `references/designs/officecli-design-redgold.md`
- No style matched -> follow `officecli load_skill pptx` defaults (no extra design doc).

The `officecli-design-*.md` files are visual skeletons (master layout with px coordinates, hex palette + area ratios, font size steps, density gates, page-type rules, hard gates) -- apply them via `officecli add --type shape/text` and `set`, then verify with `view html/screenshot`. The original `design-principle.*.md` copies are preserved in the same folder for reference.
